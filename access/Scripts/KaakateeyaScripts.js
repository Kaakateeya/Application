...
function checkEmail1(obj) {
    ...
    var matchArray = emailStr.match(emailPat);
    if (matchArray == null) {
        jAlert("You must supply a valid email address1.", "Alert Dialog", obj);
        obj.value = "";
        obj.focus();
        return false;
    }
    ...
    for (i = 0; i < user.length; i++) {
        if (user.charCodeAt(i) > 127 || user.charCodeAt(0) == 95 || (user.charCodeAt(0) > 47 && user.charCodeAt(0) < 58)) {
            jAlert("The supplied email user name contains invalid characters1.", "Alert Dialog", obj);
            obj.value = "";
            obj.focus();
            return false;
        }
...
function checkEmail2(obj) {
    ...
    if (checkTLD && domArr[domArr.length - 1].length != 2 && domArr[domArr.length - 1].toLowerCase().search(knownDomsPat) == -1) {
        alert('The supplied email address must end in a well known domain or two letter');
        obj.value = "";
        obj.focus();
        return false;
    }

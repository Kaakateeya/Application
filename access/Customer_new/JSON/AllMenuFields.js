var flagstartindex = 1, flagEndindex = 10, SelfData, count = 0, names = "", SearchRequestText, Empid, TotalRow, rowss, FilterArray = [], Typeofbind;
...
$(function () {
    ...
    $("#ddlModeofservice").on('change', function () {
        if (getvalues("#ddlModeofservice") != "0" && getvalues("#ddlModeofservice") == "4") {
...
function submitGeneralSearch(fromVal, Toval) {
    ...
    if (fromVal == undefined && Toval == undefined) {
        ...
        SearchRequest.genearlsearchreq = {
            ...
            EmpID: Empid,
            Educatiomemo: $('#txtEducationmemo').val() != "" ? $('#txtEducationmemo').val() : null,
            Chkeducationmemo: $("#chkeducation").attr("checked") ? 0 : 1,
            ProfessionMemo: $('#txtprofession').val() != "" ? $('#txtprofession').val() : null,
            Chkprofessionmemo: $("#chkprofession").attr("checked") ? 0 : 1
        }
        SearchRequestText = {
            ...
            Education: getvaluestext('#lsteducation'),
            ...
            LastName: $('#txtlname').val() != "" ? $('#txtlname').val() : null,
            Educatiomemo: $('#txtEducationmemo').val() != "" ? $('#txtEducationmemo').val() : null,
            Chkeducationmemo: $("#chkeducation").attr("checked") ? 1 : 0,
            ProfessionMemo: $('#txtprofession').val() != "" ? $('#txtprofession').val() : null,
            Chkprofessionmemo: $("#chkprofession").attr("checked") ? 1 : 0
        };
    ...
    var data = CallApi('AllMenuFields.aspx/GetgeneralSearchSlide', SearchRequest);
...
function condional(Array) {
    ...
    $.each(Array, function (index, item) {
        ...
        data.push({ label: 'backendFields', Custid: item.Cust_ID, ProfileID: item.ProfileID, PhotoCount: item.PhotoCount, Age: item.Age, HeightInCentimeters: item.HeightInCentimeters, MaritalStatusID: item.MaritalStatusID, CasteID: item.CasteID, serviceDate: item.serviceDate, CustPhoto: (item.imageurl).replace("~/", "../../") });
       ...
       (($('#ddlModeofservice option:selected').text() != 'Suggested by EMP') && ($('#ddlRelationship').val() == 0 || $('#txttoname').val() == ''))) {
        ...
        $('#lstselectedprofiles option').each(function () {
            ...
            var expObj = {
                ...
                EmpID: Empid,
                ModeofService: $('#ddlModeofservice option:selected').val(),
                RelationShipID: $('#ddlRelationship option:selected').val(),
                Name: $('#txttoname').val(),
                TypeOfService: $("input:radio[name='rbtntypeofservices']:checked").val(),
                ProfileType: $("input:radio[name='rbtnprofile']:checked").val(),
                NotesofCustomer: $('#txtnotesofcustomer').val(),

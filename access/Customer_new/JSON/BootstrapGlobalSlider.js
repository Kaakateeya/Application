...
//Carousal ID="myCarousel" for normal searchslide,
//            "PopupCarousal" for normal Searchslide shows in popup
//             "myCarousel" for marketing and matchFollowup
...
$(function () {
    ...
    //AdminObj = CallApi("GetLoginAdmin", {});
    //var empid = { empid: empidObj.d };
    //empNameObj = CallApi("getEmpName", empid);
    //empMobObj = CallApi("getempmobile", empid);
...
var obj = {
    ...
    InputObject: {},
    api: undefined, //'EmployeeGeneralSearchLatest.aspx/GetShorlistedProfiles',
...
function PageDesign(Inputobject) {
    ...
    switch (obj.PageType.toLowerCase()) {
        case 'general':
            Slidepageload(obj.CarousalID, "lblcurSlide", "lblnoofrecords", "lnkLastSlide", $('#' + Btns).find('button').attr("id"), $('#' + Btns).find('button').next().attr("id"), obj.resultArray[0].TotalRows);
            var datanew = condional(obj.resultArray.d);
            var totalrecords;
   ...
   + "</div>";
    ...
    + "<br>"
        ...
        case 'marketing':
            ...
            pageflag = 'market';
            var markethistry = [];
            var totalrecords = $('#' + CarousalID).find('.item').length;
            console.log('marketkjlkjkl');
            console.log(obj.resultArray.d.Marketingslideticket);
            _.each(obj.resultArray.d.Marketingslideticket, function (item) {
                totalrecords = totalrecords + 1;
                var mar = StaticArray.Marketing;
                var siblingsflag = 1;
                markethistry = _.where(obj.resultArray.d.MarketingslideHistory, { Emp_Ticket_ID: (item.Emp_Ticket_ID) });
                _.map(markethistry, function (item1, index) {
                    item1.ReplyDatenew = moment(item1.ReplyDatenew).format('YYYY/MM/DD h:mm a')
                });
                console.log(_.sortBy(markethistry, 'ReplyDatenew'));
                markethistry = _.sortBy(markethistry, 'ReplyDatenew').reverse();
                var TicketHistry;
                _.each(markethistry, function (histryItem) {
                    TicketHistry += StaticArray.MatchFollowUp.returnhtml(histryItem.TicketType, histryItem.ReplyDate, histryItem.NAME, histryItem.CallStatus, (histryItem.CallReceivedBy) + (histryItem.RelationShip != '' ? " ( " + histryItem.RelationShip + " )" : ""), histryItem.ReplyDesc, histryItem.NoOfDays, histryItem.MatchmeetingStatus)
                });
                item.CustID = 81668;
                var strTicketHiostryUpdate = "";
                var guestlinkshide = (item.CustID) != null && (item.CustID) != "" ? "display:block;" : "display:none;";
                var guestlinkshideclass = (item.CustID) != null && (item.CustID) != "" ? "" : "Linkdisabledforsaform";
                totalrecords = totalrecords + 1;
                var strreminderclass = parseInt(item.AssignedToEmpID) == parseInt(empidObj) ? 'fa fa-pencil-square-o' : 'fa fa-pencil-square-o Linkdisabled';
                var strreminderDate = (item.CustID) != null && (item.CustID) != "" ? (item.ReminderDate == "" ? "<b><f class='maroon' id='lblcreateremainder" + totalrecords + "'>Create Reminder </f> <label id='lblcreateremainderdate" + totalrecords + "' style='display:none;'></label>&nbsp; <a  class='" + strreminderclass + "' title='Create Reminder' href='javascript:void(0)' onclick='updatereminder(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileID) + "," + JSON.stringify(item.TicketID) + "," + JSON.stringify(item.ReminderDatepopup) + "," + JSON.stringify(item.ReminderID) + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + totalrecords + ");'></a></b>"
                    : "<b><f class='maroon'>Reminder date:</f>  <label id='lblreminderdate" + totalrecords + "'>" + item.ReminderDate + "</label>&nbsp; <a class='" + strreminderclass + "' href='javascript:void(0)' title='Update Reminder' onclick='updatereminder(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileID) + "," + JSON.stringify(item.TicketID) + "," + JSON.stringify(item.ReminderDatepopup) + "," + JSON.stringify(item.ReminderID) + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + totalrecords + " ," + JSON.stringify(item.ReminderTime) + "," + JSON.stringify(item.Category) + ");'></a></b>") : "";

                var strlastloginDate = item.Lastlogin == "" ? "No logins since registration,click forgot password" : "<c class='maroon'>Last login Date</c>  <c>" + item.Lastlogin + "</c>";

                var onlinepayment = item.onlinePayment == 0 ? "<br/><br/><label>Online:Unpaid</label>" : "<label class='control-label'>Online</label> : rs " + item.onlinePayment + "<br/>";
                var offlinepayment = item.offlinePayment == 0 ? "<label>Offline:Unpaid</label>" : "<label class='control-label'>Offline</label> : rs " + item.offlinePayment + "";
                var strpayment = "<br/><br/>" + onlinepayment + "" + offlinepayment + " ";
                var strphotorequest = (item.Photo).indexOf("noimage") != -1 && (item.Photo).indexOf("_Rev") == -1 ? "<br/><a href=javascript:void(0); class='btn btn-success-outline' onclick='return photoRequest(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileID) + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.TicketID) + "," + totalrecords + ");'>Photo Request</a><br/>" : "";
                var strforgotPwd = item.Lastlogin == "" ? "<br/><a href=javascript:void(0); class='btn btn-success-outline' onclick='return forgotPWD(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.TicketID) + "," + totalrecords + ");'>Forgot Password</a>" : "";
                var totalslides = $('#BothsideCarousel').find('.item').length;
                if (flagserves != 1) {
                    var lblcurrentprofile = $('#lblcurrentprofile').text();
                }

                var classdisplaytxtSA = item.SettlementValue != null && item.SettlementValue != "" ? "display:none;" : "display:block;"
                var classdisplaylblSA = item.SettlementValue != null && item.SettlementValue != "" ? "display:block;" : "display:none;"
                var strlnktextSA = item.SettlementValue != null && item.SettlementValue != "" ? "update SA" : "Save"

                var strSAAmount = "<div class='col-lg-6'> <span class='maroon' style='font-size:12px;font-weight:bold;'> Proposed SA Fee</span><br/> <div id='SAEditdiv" + totalrecords + "' style='" + classdisplaylblSA + "'><label id='lblSAamount" + totalrecords + "'>" + item.SettlementValue + "</label>&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick='return editSAamount(" + totalrecords + ");'> Edit </a></div>  <div id='SAupdatediv" + totalrecords + "' style='" + classdisplaytxtSA + "'><input type='text' class='form-control'  id='txtSAamount" + totalrecords + "' style='width:80px;float:left;' onkeydown='return !(event.keyCode == 32);' style='width:80px;float:left;' maxlength='7' value='" + item.SettlementValue + "'/>&nbsp;&nbsp;&nbsp;<a id='lnksaupdate" + totalrecords + "' href='javascript:void(0);' onclick='return UpdateSAamount(" + totalrecords + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.CustID) + ");'>" + strlnktextSA + " </a></div></div>";

                var Saformdisabled = (item.settleddeleted != "") && (item.settleddeleted != null) ? " " : " Linkdisabledforsaform";
                var SaformdisabledText = (item.settleddeleted != "") && (item.settleddeleted != null) ? "Show SA Form" : "No SA Form";
                var sibblinglinks = (item.CustID) != null && (item.CustID) != "" ? "<a href='javascript:void(0);' class='btn btn-success-outline " + Saformdisabled + "' onclick='return showSettledForm(" + item.settleddeleted + ");'>" + SaformdisabledText + "</a><br/><br/><a href='javascript:void(0);' class='btn btn-success-outline' onclick='return Deletesettlereason(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileStatusID) + ");'>Set/Del Reason</a><br/><br/>"
                    + "<a href='javascript:void(0);' class='btn btn-success-outline' onclick='radioChangeSibling(" + JSON.stringify("Y") + "," + totalrecords + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.CustID) + ");'>Sibling Created</a><br/><br/>"
                 + "<a href='javascript:void(0);' class='btn btn-success-outline'  onclick='radioChangeSibling(" + JSON.stringify("N") + "," + totalrecords + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.CustID) + ");'>Do not Distrub</a><br/>" + strpayment + "" : "";
                var classdisplaytxt = item.Feedetails != 0 && item.Feedetails != null && item.Feedetails != "" ? "display:none;" : "display:block;"
                var classdisplaylbl = item.Feedetails != 0 && item.Feedetails != null && item.Feedetails != "" ? "display:block;" : "display:none;"
                var strlnktext = item.Feedetails != 0 && item.Feedetails != null && item.Feedetails != "" ? "update Fee" : "Save"

                var strfeediv = siblingsflag != 1 ? "<div class='row' style='" + guestlinkshide + "'><div class='col-lg-6'><span class='maroon' style='font-size:12px;font-weight:bold;'>Proposed Reg Fee</span><br/>  <div id='feeEdit" + totalrecords + "' style='" + classdisplaylbl + "'><label id='lblfee" + totalrecords + "'>" + item.Feedetails + "</label>&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);'  onclick='return editFee(" + totalrecords + ");'>Edit </a></div>  <div id='feeupdate" + totalrecords + "' style='" + classdisplaytxt + "'><input type='text' class='form-control'  id='txtfees" + totalrecords + "' style='width:80px;float:left;' onkeydown='return !(event.keyCode == 32);' style='width:80px;float:left;' maxlength='7' value='" + item.Feedetails + "'/>&nbsp;&nbsp;&nbsp;<a id='lnkfeeupdate" + totalrecords + "' href='javascript:void(0);' onclick='return UpdateFee(" + totalrecords + "," + JSON.stringify(item.Emp_Ticket_ID) + "," + JSON.stringify(item.CustID) + ");'>" + strlnktext + " </a></div></div>   " + strSAAmount + "   </div><br/>" : "";

                var strProfileStatus = siblingsflag == 1 ? (item.ProfileStatusID == "56" || item.ProfileStatusID == "394" ? "<div class='vertical-text' style='background-color: rgb(160, 2, 1);'>DELETED</div>" : (item.ProfileStatusID == "57" || item.ProfileStatusID == "393" ? "<div class='vertical-text' style='background-color: green;'>SETTLED</div>" : "")) : "";

                var strMissingFields = siblingsflag != 1 && (item.CustID != null && item.CustID != "") ? "<a href='javascript:void(0);' id='lnkmissingmail' onclick='return Missingfieldsemail(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileID) + ");' style='font-weight: bold;" + guestlinkshide + "'>Send Full Profile Tocustomer</a>" : "";
                var strmaskclass = siblingsflag == 1 ? (item.ProfileStatusID == "56" ? "Deleted" : (item.ProfileStatusID == "57" ? "Settled" : "")) : "";
                var stremployeeimg = (item.EmpPhoto != null) && (item.EmpPhoto != "") ? (item.EmpPhoto).replace("~/", "../../") : "";
                var strcontactview = (item.CustID) != null && (item.CustID) != "" ? "<a href='javascript:void(0);' onclick='return redirecttoContactPage(" + (item.CustID) + "," + (empidObj) + "," + (AdminObj) + ");'>  ViewContacts/Edit Profile</a>" : "&nbsp;";
                var leftblock = "<div class='col-lg-6' style='border-radius: 20px'>"
    + "    <div>"
    + "        <div class='row'>"
    + "            <div class='col-lg-3'>"
    + empPhoto(item.EmpPhoto, item.EmpName)
                + mar.marketSidemenu('Send SMS', "return sendSms('" + item.PrimaryContactNumber + "','" + item.CustomerName + "','" + item.PrimaryEmail + "','" + item.PrimaryContactNumberCountyCode + "','" + item.Emp_Ticket_ID + "','" + item.TicketID + "','" + totalrecords + "','Updatedivto','lnkAddactionTo');", guestlinkshide)
                + mar.marketSidemenu('Send Mail', "return sendMail('" + item.ProfileID + "','" + item.Emp_Ticket_ID + "','" + item.TicketID + "','" + totalrecords + "','Updatedivto ','lnkAddactionTo','" + undefined + "','" + undefined + "','" + undefined + "','" + item.CustomerName + "','" + item.PrimaryEmail + "');", guestlinkshide)
                                + strphotorequest
                                + strforgotPwd
    + "        </div>"
    + "        <div class='col-lg-8 text-center'>"
    + "            <div>" + strMissingFields + "</div>"
    + strProfileStatus
    + "            <input id='CustImg" + totalrecords + "' onclick='return getimgPath(" + JSON.stringify(item.CustID) + "," + JSON.stringify(item.ProfileID) + "," + JSON.stringify(item.PhotoCount) + ")' src='" + item.Photo + "' class='img-thumbnail' style='border: 1px solid gray;' type='image'><div class='text-center' style='line-height: 22px;'>"
    + "                <br>"
    + CustNameWithProfileID(item.CustomerName, item.ProfileID, item.RegistrationDate, guestlinkshideclass) + "<br>"
    + "                <div style='" + guestlinkshide + "'><b>"
    + "                    <f class='maroon'>DOR:</f>"
    + "           " + item.RegistrationDate + "</b></div>"
    + "        <br>"
        + strreminderDate
    + "</div>"
    + "</div>"
    ...
    + "</div>"
    + "<br>"
    + "<div class='col-lg-12 col-lg-push-2' style='border: 1px solid; border-color: orange; border-radius: 20px; width: 81%; display: block;" + guestlinkshide + "'>"
    + mar.CountsDiv(strlastloginDate, item.RectViewedProfCount, item.RectWhoViewedCout, item.MybookMarkedProfCount, item.WhobookmarkedCount, item.IgnoreProfileCount)
    + "</div>"
    + "</div>";

                var strmobileverified = item.IsMobileVerified == "True" ? "&nbsp;<span class='fa fa-check-circle'></span>" : "&nbsp;<span class='glyphicon glyphicon-ok'></span>";
                var stremailverified = item.isEmailVerified == "True" ? "&nbsp;<span class='fa fa-check-circle'></span>" : "&nbsp;<span class='glyphicon glyphicon-ok'></span>";
                var rightBlock = "<div class='col-lg-6' style='border: 1px solid; border-color: orange; border-radius: 20px;'>"
    + "    <div style=''>"
    + "        <div class='text-center'><b><u><a href='javascript:void(0);' id='lblticID" + totalrecords + "' onclick='ticketidaction(" + JSON.stringify(item.Emp_Ticket_ID) + ");'>" + item.TicketID + "</a></u>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' onclick='return redirecttoContactPage(" + (item.CustID) + "," + (empidObj) + "," + (AdminObj) + ");'>  ViewContacts/Edit Profile</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label>Delayed : </label>"
    + "            <label>" + item.NoofDays + "</label></b></div>"
    + "        <br>"
    + "        <div class='row'>"
    + mar.CustMobEmail('Mobile', (item.PrimaryContactNumberCountyCode != "" ? (item.PrimaryContactNumberCountyCode + " - ") : "") + "" + item.PrimaryContactNumber + strmobileverified)
    + mar.CustMobEmail('Email', item.PrimaryEmail + stremailverified)
    + "        </div>"
    + "    </div>"
    + "    <br>"
    + "    <div class='row' style='" + guestlinkshide + "'>"
    + mar.FeeSaDiv('Proposed Reg Fee', 'feeEdit1', 'lblfee1', '233', 'return editFee(1);', 'feeupdate1', 'txtfees1', 'lnkfeeupdate1', 'return UpdateFee(1,&quot;1736&quot;,&quot;54901&quot;);')
    + mar.FeeSaDiv('Proposed SA Fee', 'SAEditdiv1', 'lblSAamount1', '2433', 'return editSAamount(1);', 'SAupdatediv1', 'txtSAamount1', 'lnksaupdate1', 'return UpdateSAamount(1,&quot;1736&quot;,&quot;54901&quot;);')
    + "    </div>"
    + "    <br>"
    + "    <div id='Updatediv" + totalrecords + "' style='overflow: scroll; height: 600px;'>"
    + TicketHistry
    + "</div>"
    + "<br>"
    + "<div></div>"
    + "</div>";
                AppendCarousalItems(CarousalID, leftblock, rightBlock);
                document.getElementById("CustImg" + totalrecords).disabled = (item.Photo).indexOf("noimage") == -1 && ((item.CustID) != null && (item.CustID) != "") ? false : true;
            });
            break;
    }
    return false;

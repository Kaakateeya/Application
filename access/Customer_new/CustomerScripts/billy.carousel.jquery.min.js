...
*/
(function($){$.fn.extend({billy:function(options){var defaults={scrollSpeed:500,slidePause:4000,indicators:$('section#billy_indicators'),indicatorLinks:true,activeClass:'active',scrollAmount:'auto',nextLink:$('#billy_next'),prevLink:$('#billy_prev'),autoAnimate:true,loop:true,customIndicators:false,noAnimation:false,};var options=$.extend(defaults,options);var option;var object;var slides;var slidewidth;var slidecount;var currentslide;var defautindicator;var the_indicators;var hreftag;var indicatorsli;var currentindicator;return this.each(function(){option=options;object=$(this);slides=object.find('article');if(option.scrollAmount=='auto'){slidewidth=slides.width();}else{slidewidth=option.scrollAmount;}
slidecount=(slides.width()*slides.length)/slidewidth;slidecount=Math.round(slidecount);currentslide=0;if(slides.length>0){if(!option.customIndicators){for(var i=0;i<slidecount;i++){if(!option.indicatorLinks){option.indicators.append('<article></article>');}else{option.indicators.append('<article><a href="#'+i+'"></a></article>');}}}
defautindicator=option.indicators.find('article:eq(0)');defautindicator.addClass(option.activeClass)
the_indicators=option.indicators.find('article a');the_indicators.click(function(){hreftag=$(this).attr('href');hreftag=hreftag.substring(hreftag.search('#')+1,hreftag.length);jumptospecific(hreftag);if(option.autoAnimate){clearInterval(period);period=window.setInterval(function(){if(currentslide>=(slidecount-1)){jumptostart();}else{jumpnext();}},option.slidePause);}
